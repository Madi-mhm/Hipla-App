 ...
 *
 * Le gabarit et l'adaptateur sont ceux de la facture. Un devis partage
 * tout avec elle — émetteur, destinataire, lignes, ventilation par taux —
 * et n'en diffère que par l'intitulé, la date affichée et l'absence de
 * coordonnées bancaires. Un second gabarit aurait dupliqué trois cents
 * lignes pour trois différences, et les aurait laissées diverger.
 */
...
export async function GET(
  ...
  if (erreur || !modele) {
    ...
    return NextResponse.json(
      { erreur: erreur ?? 'Génération du devis impossible.' },
      { status: 422 }
    );
  }
